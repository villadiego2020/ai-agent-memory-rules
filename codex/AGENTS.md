...
Codex profiles use a deliberate model/effort tier: `gpt-6-luna/low` for narrow 3D and Memory scans, `gpt-6-sol/medium` for implementation and architecture, `gpt-6-sol/high` for network and verification, and `gpt-6-astra/medium` for cross-system planning. Explicit user or spawn settings still take precedence. Do not copy Claude-only model aliases into Codex profiles.
